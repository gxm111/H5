module junyou.sui {
    ...
    export class IconButton extends Button {
		...
		private initComponent() {
			...
			this._iconLayout = Layout.TOP_LEFT;
		}
		 ...
		 * 支持layout
		 */
		public set icon(value: sui.Image) {
			this._icon = value;
			if (value) {
		...
		public get icon(): sui.Image {
			return this._icon;
		}

		/*图标加载完成后layout*/
		private iconComplete(e: egret.Event) {
			this._icon.removeEventListener(egret.Event.COMPLETE, this.iconComplete, this);
			this.updateDisplayList();
		}
		...
		public set iconSource(value: string) {
			if (this._iconSource == value)
				return;
			this._icon.addEventListener(egret.Event.COMPLETE, this.iconComplete, this);
