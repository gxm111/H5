module junyou.sui {
	...
	export class ToolTipManager {
		...
		private static _pool: RecyclablePool<ToolTipData> = new RecyclablePool(ToolTipData);

		private static _map: Map<egret.DisplayObject, ToolTipData> = new Map<egret.DisplayObject, ToolTipData>();
		...
		public static register(dis: egret.DisplayObject, msg: any, tooltip?: IToolTip, container?: egret.DisplayObjectContainer) {
			...
			if (!data) {
				data = ToolTipManager._pool.getInstance();
				map.set(dis, data);
			}
			if (!container) {
				container = game.GameEngine.instance.getLayer(game.GameLayerID.Tip);
			}
		...
		/**
		 * 刷新当前Tip绑定的内容，<font color="#f00">不改变显示状态</font><br/>
		...
		public static remove(dis: egret.DisplayObject) {
			let data = ToolTipManager._map.get(dis);
			ToolTipManager._pool.recycle(data);
		}
