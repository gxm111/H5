module junyou.mvc {
    ...
    export abstract class Mediator extends FHost {
        ...
        public set view(value: IModulePanel) {
            if (this._view != value) {
                ...
                var old = this._view;
                if (old) {
                    old.removeEventListener(Event.REMOVED_FROM_STAGE, this.stageHandler, this);
                    old.removeEventListener(Event.ADDED_TO_STAGE, this.stageHandler, this);
                }
                this._view = value;
                value.moduleID = this._name;
                value.addEventListener(Event.REMOVED_FROM_STAGE, this.stageHandler, this);
                value.addEventListener(Event.ADDED_TO_STAGE, this.stageHandler, this);
                if (isIAsync(value)) {
        ...
        protected stageHandler(e: Event) {
            ...
            if (e.type == Event.ADDED_TO_STAGE) {
                ...
                for (type in _interests) {
                    ins = _interests[type];
                    facade.addEventListener(type, ins.handler, this, false, ins.priority);
                    if (ins.trigger) {
                ...
                for (type in _interests) {
                    ins = _interests[type];
                    facade.removeEventListener(type, ins.handler, this);
                }
                this.sleep();
            }
        }
     ...
     * @param {number} [priority=0]                 优先级，默认为0
     */
    export function interest(eventType: string, triggerOnStage?: boolean, priority?: number) {
