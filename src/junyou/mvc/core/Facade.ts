module junyou.mvc {
    ...
    export class Facade extends egret.EventDispatcher {
         ...
         * @returns
         */
        public static hasEventListener(type: string) {
         ...
         */
        public static addEventListener(type: string, listener: Function, thisObject: any, priority?: number) {
            Facade._instance.addEventListener(type, listener, thisObject, false, priority);
        }
        ...
        public static removeEventListener(type: string, listener: Function, thisObject: any) {
            Facade._instance.removeEventListener(type, listener, thisObject, false);
        }
         ...
         */
        public registerInlineProxy(ref: { new (): Proxy }, proxyName?: string, async?: boolean) {
         ...
         */
        public registerInlineMediator(ref: { new (): Mediator }, mediatorName?: string) {
            let className = egret.getQualifiedClassName(ref);
            if (!mediatorName) {
		 ...
		 */
        public registerProxyConfig(className: string, proxyName: string, scriptid?: string) {
		 ...
		 */
        public registerMediatorConfig(className: string, moduleID: string, scriptid?: string) {
         ...
         */
        public getProxy(proxyName: string, callback: { (proxy: Proxy, args?: any[]) }, thisObj: any, ...args) {
            let dele = this._proxys[proxyName];
            if (DEBUG) {
                if (!dele) {
         ...
         * @param args 回调函数的参数列表
         */
        public getMediator(moduleID: string, callback: { (mediator: Mediator, args?: any[]) }, thisObj: any, ...args) {
        ...
        public toggle(moduleID: string, show = -1, showTip = true) {
            if (this._mm) {
                this._mm.toggle(moduleID, show, showTip);
            }
        }
         ...
         * @returns
         */
        public executeMediator(moduleID: string, showTip: boolean, handlerName: string, show?: boolean, ...args) {
            if (DEBUG) {
        ...
        protected _executeAndShowMediator(mediator: Mediator, args: any[]) {
            this.toggle(mediator.name, 1, false);//showTip为 false是不用再次提示，executeMediator已经执行过模块是否开启的检查
            let handlerName = args[0];
            if (typeof mediator[handlerName] === "function") {
                (<Function>mediator[handlerName]).apply(mediator, args);
            } else if (DEBUG) {
         ...
         * @param args          参数列表
         */
        public executeProxy(proxyName: string, handlerName: string, ...args) {
        ...
        protected _executeProxy(proxy: Proxy, args: any[]) {
            ...
            if (typeof proxy["handlerName"] === "function") {
                ...
                ThrowError("无法在Mediator：" + proxy.name + "中找到方法[" + handlerName + "]");
            }
        }
...
module junyou {
    ...
    export function dispatch(type: string, data?: any) {
        mvc.Facade.getInstance().dispatchEventWith(type, false, data);
    }
}