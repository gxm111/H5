module junyou {
    ...
    export class DateUtils {
		...
		public static initServerTime(time: number, timezoneOffset: number) {
			DateUtils._utcOffset = -timezoneOffset * DateUtils.ONE_MINUTE;
		...
		public static getCountdown(leftTime: number, format: { d?: string, h?: string, m?: string, s?: string }) {
			...
			let tmp = format.d;
			if (tmp) {// 需要显示天
				let day = leftTime / 86400000;//DateUtils.ONE_DAY;
				leftTime = leftTime - day * 86400000;
				out += tmp.substitute(day);
			}
			tmp = format.h;
			if (tmp) {// 需要显示小时
				let hour = leftTime / 3600000;//DateUtils.ONE_HOUR
				leftTime = leftTime - hour * 3600000;
				out += tmp.substitute(hour);
			}
			tmp = format.m;
			if (tmp) {// 需要显示分钟
				let minute = leftTime / 60000;//DateUtils.ONE_MINUTE
